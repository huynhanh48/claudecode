---
description: Scaffold a new CRUD resource end-to-end (model → schema → repository → service → controller → route + wiring + Alembic + tests).
...
5. Generate the Alembic migration: `alembic revision --autogenerate -m "add <resources> table"`.
...
8. Report the file list, migration revision id, and test result.
